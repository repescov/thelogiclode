...
class FractionAddition(MovingCameraScene):
    def construct(self):
        ...
        self.add(watermark) # Simplified: just add it, zoom will be subtle

        # 3. CONTINUOUS SLOW ZOOM
        self.camera.frame.save_state()
        self.camera.frame.set(width=config.frame_width * 1.1)

        # --- INTRO (The Hook) ---
        intro_text = Text("How do we add fractions?", font_size=45, color=BLUE_B).to_edge(UP, buff=1.5)
        ...
        expl2 = Text("One denominator is a multiple of the other", font_size=25, color=BLUE_A).to_edge(DOWN, buff=2)
        ...
        self.play(Create(v2a_extra), ReplacementTransform(f2a, f2a_split), run_time=1.5)
        self.wait(1)
        ...
        self.play(Create(h3a_extra), ReplacementTransform(f3a, p3a_split), run_time=1.5)
        self.play(Create(v3b_extra), ReplacementTransform(f3b, p3b_split), run_time=1.5)
        self.wait(1)
        ...
        # --- SCENE 4: 1/4 + 1/6 (LCM) ---
        title4 = Text("4) Least Common Multiple\n     (LCM)", font_size=45, color=GRAY_A).to_edge(UP, buff=1.2)
        ...
        self.play(
            Write(title4), Write(eq4), Write(lcm_hint),
            self.camera.frame.animate.set(width=config.frame_width * 1.1),
            run_time=2,
            rate_func=linear
        )
        self.wait(1)
        ...
        self.play(ReplacementTransform(v4a, v4a_12), ReplacementTransform(f4a, p4a_split), run_time=1.5)
        self.play(ReplacementTransform(v4b, v4b_12), ReplacementTransform(f4b, p4b_split), run_time=1.5)
        self.wait(1)
        ...
        self.play(
            ...
            self.camera.frame.animate.set(width=config.frame_width * 1),
            run_time=2
        )
